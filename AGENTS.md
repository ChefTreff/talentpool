...
- Service-Role-Diagnose: `node --env-file=.env.local scripts/<script>.mjs`. Gültigkeit des lokalen Secret Keys prüfen (ohne ihn auszugeben): `node --env-file=.env.local scripts/check-secret.mjs`.
