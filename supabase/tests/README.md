...
| `v2_roles_admin.sql` | 0022 | Admin-Lesewege: `applications_for_session` (Entscheider, 42501 sonst), `applications_overview`, `roles_of_person`, `assign_role` (idempotent, Vokabular, Scope-Check), `revoke_role` (Ablaufdatum, letzter Admin geschützt), `search_people` (Team; E-Mail nur Admin), Audit-Einträge |
