-- 0075 · vivenu-Ingest: veraltete Stände verwerfen, unbekannte Tickettypen nicht raten (Antworten der Architektur-Session zu PR #24).
--   F8 Replay/Out-of-order: die Webhook-Id schützt gegen dieselbe Zustellung, nicht gegen eine **ältere**, die nach einer
--      neueren ankommt. `ticket.vivenu_updated_at` merkt sich den Stand von vivenu; ist der eingehende älter, wird er ignoriert.
--   F9 Unbekannter Tickettyp: Tickets dürfen nie verloren gehen. Statt `pass_type = 'professional'` zu raten, bleibt die
--      Spalte leer; die Route meldet je unbekanntem Typ **einmal** einen Sync-Fehler.
-- HINWEIS: der Nachtrag `backfill_ticket_pass_types()` aus dieser Migration suchte an der falschen Stelle und tat nichts —
-- korrigiert in 0076. Die Funktionsfassungen hier werden von 0076–0078 ersetzt; angewendete Migrationen bleiben unverändert stehen.
-- Abweichungen: keine.
set search_path = public, extensions;

alter table ticket add column if not exists vivenu_updated_at timestamptz;
comment on column ticket.vivenu_updated_at is 'Stand des Tickets bei vivenu (updatedAt). Ältere Webhooks werden dagegen verworfen.';
