...
--
-- Anlass: ADM-057, uebergeben aus dem Partner-Chat (PART-059). Die Kundennummer steht seit 0164 in
-- `organization.customer_number`; bis heute pflegt sie das Partner-Team von Hand im Admin
-- (`set_org_customer_number`), obwohl HubSpot sie fuehrt.
--
-- **Welche Eigenschaft gilt.** Konrad, 24.09.2026: `company_id` — im Portal „Uebergreifende
-- Kundennummer (Company ID)". Am 25.09.2026 lesend gegen HubSpot geprueft: die Eigenschaft
-- existiert, Typ Text, 3542 Firmen, davon 41 ohne Nummer, Form `C-#####`. Die zweite Kandidatin
-- `cheftreff_id_unternehmen` („synchronisiert mit Sevdesk und AirTable", Typ Zahl) gilt nicht.
--
-- **Nur ergaenzen, nie ueberschreiben** — dieselbe Regel wie fuer die uebrigen Firmendaten. Steht
-- bei uns schon eine Nummer, bleibt sie; weicht HubSpot ab, steht das im Audit-Eintrag
-- (`customer_number_conflict`), damit es niemand still ausbuegelt.
--
-- **Doppelte Nummern sind ein Gate-Fehler.** Derselbe Wert an zwei Firmen ist ein Tippfehler in
-- HubSpot, der spaeter Belege falsch zuordnet; der Teilindex `organization_customer_number_key`
-- (0164) laesst ihn ohnehin nicht zu. Statt eines 23505 mitten im Ingest meldet das Gate
-- `customer_number_taken:<nummer>` — mit Fehlerdatensatz, Mail an den Deal-Owner und Slack, wie
-- jeder andere Gate-Fehler. Das ist kein Randfall: in der Stichprobe vom 25.09.2026 haengen 35
-- Nummern an mehr als einer Firma.
--
-- Rechte und Fehlerschluessel bleiben, wie sie waren (`auth.uid() is null`, Schluessel
-- `customer_number_taken` schon in `lib/rpc-error.ts` und beiden Woerterbuechern registriert).
-- Basis: `supabase/snapshot/functions/ingest_partner_deal.sql` (Konvention §1).
-- Test: `supabase/tests/v6_kundennummer_ingest.sql`.
