-- 0076 · Tickettyp merken, damit der Nachtrag wirklich nachtragen kann (Korrektur an 0075).
-- 0075 hat `backfill_ticket_pass_types()` gegen `ticket.extra_fields->>'ticketTypeId'` gesucht. Dort steht die Typ-Id nie:
-- `extra_fields` sind die Zusatzfelder von vivenu, nicht die Ticketdaten. Die Funktion hätte stillschweigend nichts getan —
-- schlimmer als kein Nachtrag, weil man sich darauf verlassen hätte. Jetzt merkt sich `ticket.vivenu_ticket_type_id` den Typ.
-- Abweichungen: keine.
set search_path = public, extensions;

alter table ticket add column if not exists vivenu_ticket_type_id text;
comment on column ticket.vivenu_ticket_type_id is 'Tickettyp bei vivenu. Grundlage für den Nachtrag des Pass-Typs, wenn ticket_type_map später gefüllt wird.';
create index if not exists ticket_vivenu_type_idx on ticket (vivenu_ticket_type_id) where pass_type is null;
...
/** Pass-Typen nachtragen, sobald `ticket_type_map` gefüllt ist. Rührt nur Tickets ohne Pass-Typ an. */
create or replace function backfill_ticket_pass_types() returns integer
language plpgsql volatile security definer set search_path = public, extensions as $$
declare v_n integer;
begin
...
revoke execute on function backfill_ticket_pass_types() from public, anon, authenticated;
