-- 0077 · Ingest: fehlender Personalisierungsstand bricht nicht mehr ab (Fund im eigenen Smoke-Test zu 0076).
-- `ticket.personalization_status` ist NOT NULL. Schickt vivenu das Feld nicht mit, wäre der Insert mit 23502 gescheitert,
-- vivenu hätte siebenmal wiederholt und das Ticket wäre verloren. Beim Anlegen gilt jetzt `pending` als Rückfall.
-- HINWEIS: dieselbe Falle steckte in `addons` und `currency` — 0078 sichert alle NOT-NULL-Spalten auf einmal ab.
-- Abweichungen: keine.
