-- 0171 · person.pronouns und person.cv_url streichen (SPK-066, TAL-013 B3)
-- Angewendet von der Architektur-Session am 24.09.2026 als 20260924190700.
--
-- Anlass: SPK-066 (Konrad 24.09.: Pronomen raus, „das Feld kann gelöscht werden“; das Speaker-Formular
-- schickt seit #176 kein `pronouns` mehr, das Teilnehmer-Profil nutzte die Spalte nie) und TAL-013 B3
-- (Lebenslauf als Datei `cv_path` im privaten Bucket, 0161 — die URL-Spalte `cv_url` hatte nie einen
-- Weg in der Oberfläche).
--
-- Befund vor dem Streichen (Architektur-Session, 24.09., live): 25 Personen, 0 Werte in beiden Spalten;
-- keine View, keine Policy hängt daran; nur drei Funktionen lesen oder schreiben sie. Deren Fassungen
-- unten sind die Live-Fassungen aus supabase/snapshot/functions/, jeweils nur um den betroffenen
-- Ausdruck gekürzt (my_speaker_profile: Schlüssel `pronouns` im Personenobjekt; update_my_speaker_profile:
-- Zuweisung `pronouns`; anonymize_person: `cv_url = null`, `pronouns = null`). Ein `pronouns` im
-- Payload von update_my_speaker_profile wird ab jetzt stumm ignoriert.
--
-- Fehlerschlüssel: unverändert. Test: supabase/tests/v6_pronomen_cv_url.sql
...
alter table person drop column if exists pronouns;
alter table person drop column if exists cv_url;
