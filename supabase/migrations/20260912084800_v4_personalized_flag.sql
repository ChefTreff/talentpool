-- 0080 · Personalisierung: vivenu meldet `personalized`, nicht `personalizationStatus`.
--
-- Im Sandbox-Lauf am 12.09. personalisiert (`POST /tickets/personalize/{id}/{secret}`):
-- Der `ticket.updated`-Webhook kam an, Vor- und Nachname wurden übernommen — aber
-- `personalization_status` blieb auf `pending`. Im Payload steht kein
-- `personalizationStatus`, sondern `personalized: true`. Folge im Betrieb: das
-- Portal hätte den Inhaber weiter zur Personalisierung aufgefordert und der
-- Check-in hätte ein fertiges Ticket als offen geführt.
--
-- `partial`, nicht `complete`: vivenu kennt nur Namen, unser `complete` verlangt
-- zusätzlich die Badge-Angaben. Umgekehrt darf ein `partial` von dort ein
-- bereits erreichtes `complete` nicht zurückstufen.
--
-- Abweichungen: keine.
