...
--
-- Rahmen: 0117 (`v6_aufraeumen_feldmatrix`) hat `person.photo_url` entfernt — „Profilfotos
-- nur als Datei mit Rechten". Deshalb **kein** offener URL-Wert, sondern ein Pfad in einem
-- **privaten** Bucket; gezeigt wird über signierte URLs.
--
--   * `person.photo_path` — Pfad im Bucket `person-photos`, `null` = kein Porträt. Kein
--     Spalten-Grant zum Schreiben: gesetzt wird nur über `set_my_photo`.
--   * Bucket `person-photos` privat, 5 MB, PNG/JPEG/WebP (wie `contact-photos`).
--     Pfad `<person_id>/<datei>`: genau zwei Segmente.
--   * `person_photo_path_allowed(name, write)`: schreiben/löschen nur die Person selbst,
--     lesen zusätzlich das Team (`is_staff()`, Admin-Vollständigkeit: Porträt in der
--     Personenansicht).
--   * `set_my_photo(p_path)`: prüft Pfad und Objekt, setzt die Spalte, meldet das alte
--     Bild in `storage_purge_queue` an (reason `photo_replaced`); `null` entfernt.
--   * `anonymize_person` (Basis: Snapshot 20260923…): meldet das Porträt zum Wegräumen an
--     und leert `photo_path`. Sonst unverändert.
--
-- Nicht Teil: Übernahme ins Speaker-Profil (Speaker-Fotos laufen weiter über
-- `speaker_asset`); HubSpot/Swapcard bekommen das Porträt nicht.
-- Fehlerschlüssel: keine neuen (`path_mismatch`, `object_not_found` bestehen).
-- Test: supabase/tests/v6_person_portraet.sql
set search_path = public, extensions;

-- ---------------------------------------------------------------- 1 · Spalte

alter table person add column if not exists photo_path text;
comment on column person.photo_path is
  'Porträt im privaten Bucket person-photos (<person_id>/<datei>); gesetzt nur über set_my_photo (TAL-012).';

-- ---------------------------------------------------------------- 2 · Bucket + Pfadregel

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('person-photos', 'person-photos', false, 5242880, array['image/png', 'image/jpeg', 'image/webp'])
...
drop policy if exists "person photos read"   on storage.objects;
drop policy if exists "person photos insert" on storage.objects;
drop policy if exists "person photos update" on storage.objects;
drop policy if exists "person photos delete" on storage.objects;
create policy "person photos read"   on storage.objects for select to authenticated
  using (bucket_id = 'person-photos' and person_photo_path_allowed(name, false));
create policy "person photos insert" on storage.objects for insert to authenticated
  with check (bucket_id = 'person-photos' and person_photo_path_allowed(name, true));
create policy "person photos update" on storage.objects for update to authenticated
  using (bucket_id = 'person-photos' and person_photo_path_allowed(name, true))
  with check (bucket_id = 'person-photos' and person_photo_path_allowed(name, true));
create policy "person photos delete" on storage.objects for delete to authenticated
  using (bucket_id = 'person-photos' and person_photo_path_allowed(name, true));
...
begin
  ...
  if v_path is not null then
    ...
    if not exists (select 1 from storage.objects o where o.bucket_id = 'person-photos' and o.name = v_path) then
      raise exception 'object_not_found' using errcode = 'P0002';
    end if;
  end if;

  select p.photo_path into v_old from person p where p.id = v_me;
  if v_old is not distinct from v_path then return; end if;

  update person set photo_path = v_path where id = v_me;
  if v_old is not null then
    insert into storage_purge_queue (bucket, path, reason)
    values ('person-photos', v_old, 'photo_replaced')
