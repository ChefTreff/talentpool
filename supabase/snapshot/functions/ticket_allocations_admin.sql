create or replace function ticket_allocations_admin(p_edition_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(id uuid, org_id uuid, org_name text, edition_id uuid, pass_type text, quantity integer, used_count integer, coupon_code text, undershop_url text, status text, last_error text, synced_at timestamp with time zone, notes text, vivenu_coupon_id text, vivenu_undershop_id text, updated_at timestamp with time zone)
 LANGUAGE plpgsql
