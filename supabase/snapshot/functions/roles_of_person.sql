create or replace function roles_of_person(p_person_id uuid)
 RETURNS TABLE(id uuid, role text, scope_type text, scope_id uuid, edition_id uuid, portal text, scope_label text, valid_from timestamp with time zone, valid_to timestamp with time zone, active boolean, note text, granted_by uuid, created_at timestamp with time zone)
 LANGUAGE plpgsql
