...
describe("Event-App-Abbildung", () => {
  it("bildet die Org auf einen Aussteller ab: clientId = Org-ID, Beschreibung DE + EN, Website mit https, Branche", () => {
    const item = toExhibitorUpsert(row);
    assert.deepEqual(item, {
      clientId: "org1", name: "Expo", description: "Wir bauen Messen.", descriptionEn: "We build fairs.", websiteUrl: "https://expo.example",
      industry: "tech-and-it", booth: "A12",
    });
  ...
  it("gibt Level und Kategorien aus der Ableitung weiter — sie sind **nicht** die Branche", () => {
    ...
    // Das Level darf nie als `type` mitgehen: dort steht die Branche, und dabei bleibt es.
    assert.equal(toSwapcardInput(toExhibitorUpsert(row)).type, "tech-and-it");
    assert.equal(toSwapcardInput(toExhibitorUpsert({ ...row, industry: null })).type, undefined);
  });

  it("lässt die Branche weg, wenn keine angegeben ist — Swapcard behält dann, was dort steht", () => {
    assert.equal("industry" in toExhibitorUpsert({ ...row, industry: null }), false);
    assert.equal("industry" in toExhibitorUpsert({ ...row, industry: "  " }), false);
  });

  it("vergleicht die Branche gegen den Optionswert, nicht gegen die Beschriftung", () => {
    ...
    // Beim Lesen liefert Swapcard in `type` die Beschriftung — ein Vergleich damit hielte jeden Lauf für geändert.
    assert.equal(exhibitorChanged({ ...base, type: "Tech, Data & IT", typeValue: "tech-and-it" }, { ...wanted, logoUrl: undefined }), false);
    assert.equal(exhibitorChanged({ ...base, type: "Tech, Data & IT", typeValue: "fmcg" }, { ...wanted, logoUrl: undefined }), true);
    assert.equal(exhibitorChanged({ ...base, type: "Tech, Data & IT" }, { ...wanted, logoUrl: undefined }), true);
  });
  ...
  it("schreibt nur, wenn sich etwas geändert hat; Logo zählt nur mit eigenem Wert", () => {
    const wanted = toExhibitorUpsert(row);
    const same = { id: "r1", name: "Expo", description: "Wir bauen Messen.", websiteUrl: "https://expo.example", logoUrl: "https://cdn/x.png", type: "Tech, Data & IT", typeValue: "tech-and-it" };
  ...
  it("hält einen mehrzeiligen Text nicht für geändert, nur weil Swapcard ihn als HTML zurückgibt", () => {
    ...
    const remote = { id: "r1", name: "Expo", description: "Absatz AAbsatz B", websiteUrl: "https://expo.example", typeValue: "tech-and-it" };
  ...
  it("vergleicht die Standnummer nur, wenn wir sie kennen — sie hängt am Event, nicht am Aussteller", () => {
    const wanted = toExhibitorUpsert(row);
    const base = { id: "r1", name: "Expo", description: "Wir bauen Messen.", websiteUrl: "https://expo.example", logoUrl: null, typeValue: "tech-and-it" };
  ...
  it("gibt an Swapcard nur geprüfte Felder weiter: inputId = clientId, EN als Übersetzung, Branche als type, Standnummer, bestehende ID", () => {
    assert.deepEqual(toSwapcardInput(toExhibitorUpsert(row)), {
      inputId: "org1", clientId: "org1", name: "Expo", description: "Wir bauen Messen.",
      descriptionTranslations: [{ language: "en_US", value: "We build fairs." }], websiteUrl: "https://expo.example",
      type: "tech-and-it", booth: "A12",
    });
