...
- [ ] **Supabase-Integration → Vercel:** Sync auch für Preview und Development einschalten (Supabase → Integrations → Vercel → Manage), damit `SUPABASE_SECRET_KEY` und die Publishable Keys in Preview-Deployments ankommen. Die alten manuellen Dublin-Variablen sind entfernt (09.09.). **Stand 09.09. mittags:** Sync eingeschaltet, aber `SUPABASE_SECRET_KEY` und `SUPABASE_URL` liegen weiterhin nur in Production (`vercel env ls`) → in der Integration „Resync" auslösen, dann prüfen.
- [ ] **Lokaler Secret Key** (Konrad): Wert aus Supabase → Project Settings → API Keys → Secret keys einmal in `.env.local` eintragen, dann `sh scripts/env-pull.sh --worktrees`. Empfehlung: eigenen Schlüssel „local-dev" anlegen, damit er getrennt widerrufbar ist (Rotation im Entscheidungslog).
