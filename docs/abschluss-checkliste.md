...
- ☐ **Website/Sanity — Konrad, nächste Tage:** (1) drei Werte setzen: `sh scripts/env-set.sh SANITY_API_TOKEN` sowie Projekt-ID/Dataset mit `--config` (bei Vercel-Problemen zuerst `--local-only`, Vercel später nachziehen); (2) Trockenlauf mit dem Website-Team (Aufruf im Runbook `sanity-partner-logos.md`, Abschnitt Einrichtung 2) und JSON-Prüfergebnis schicken; (3) Rangfolge im Vokabular `sponsoring_level` bestätigen; (4) später Editor-Token, erster Echtlauf mit einem Logo. Zugang (Viewer) liegt seit 15.09. vor.
...
## Ergänzungen 15.09. (Rollen & Zugriff)
- ☐ **Rollendefinition im Speaker-Team am Ende durchtesten** (Konrad, 15.09.: „Lass uns die Rollendefinition im speakerTeam nochmal ganz am Ende testen"). Heute gilt: `is_speaker_team()` = `admin` **oder** `area_lead_speaker` **oder** `programme_team`; das Lead-Portal öffnet `speaker_manager` (plus `area_lead_speaker` als Bereichsleitung). Die drei Rollen bedeuten in der Praxis Verschiedenes, und ob der Schnitt stimmt, zeigt sich erst mit echten Personen — Henni (CEO, Programmverantwortung), Paulina (Head of Program), die Leads.
  Zu prüfen ist **je Rolle einzeln, mit einem Testkonto**: (1) Welche Bereiche öffnen sich (`/admin`, `/speaker-leads`, `/admin/programm`)? (2) Was ist in `manager_speakers` und `speaker_detail` sichtbar — insbesondere `internal_notes`, das nur das Team sieht? (3) Welche Team-Felder lässt `update_speaker` zu (`pass_type`, `hotel_tier`, `hospitality_status`, `owner_person_id`)? (4) Greift `approve_travel_costs` (nur Bereichsleitung/Admin)? (5) Wer darf Speaker weiterreichen (`handover_speaker`) und wer zuordnen?
  **Ein Punkt daraus ist schon entschieden** (Konrad, 15.09.: „Jetzt machen“): `programme_team` steht seither in den Rollen des Bereichs `speaker-leads` (`lib/areas.ts`, PR #49). Grund: die RPCs dahinter — `manager_speakers`, `speaker_travel_list`, `speaker_managers` — liessen das Programm-Team längst durch, nur das Bereichsgate war zu. Ein Gate, das enger ist als die Funktionen dahinter, schützt nichts; es fällt erst auf, wenn jemand vor der Tür steht. Beim Durchtesten bleibt die Frage, ob die Rolle damit **genug** und nicht **zu viel** öffnet.
- ☑ **Admin-Bereich hängt an der Rolle** statt an der Tabelle `staff_user` (Migration 0107, PR #49). Weitere Personen nimmt Konrad selbst in `/admin/team` auf (PR #48). `staff_user` und `scripts/make-staff.mjs` entscheiden nichts mehr und können gelöscht werden, sobald nichts mehr auf sie zeigt.
