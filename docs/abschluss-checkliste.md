...
- [ ] **Supabase-Integration → Vercel:** Sync auch für Preview und Development einschalten (Supabase → Integrations → Vercel → Manage), damit `SUPABASE_SECRET_KEY` und die Publishable Keys in Preview-Deployments ankommen. Die alten manuellen Dublin-Variablen sind entfernt (09.09.). **Stand 09.09. mittags nach Resync:** Production hat jetzt zusätzlich die alten Namen (`NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, sensibel); Preview und Development haben weiterhin nur URL und Publishable Key, keinen Secret Key → in der Integration die Umgebungen Preview/Development prüfen oder `SUPABASE_SECRET_KEY` dort von Hand als sensible Variable setzen (Konrad). Bis dahin: Admin-Bereich in Preview-Deployments ohne Funktion.
- [ ] **Lokaler Secret Key** (Konrad): Wert aus Supabase → Project Settings → API Keys → Secret keys einmal in `.env.local` eintragen (Zeile `SUPABASE_SECRET_KEY="sb_secret_…"`), dann `sh scripts/env-pull.sh --worktrees`. Das Skript verwirft `[SENSITIVE]`-Platzhalter grundsätzlich, sonst würde `SUPABASE_SERVICE_ROLE_KEY=[SENSITIVE]` vor dem echten Secret Key gelesen. Empfehlung: eigenen Schlüssel „local-dev" anlegen, damit er getrennt widerrufbar ist (Rotation im Entscheidungslog).
