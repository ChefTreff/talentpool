...
5. **Firmen-Eigenschaften** (interne Namen, sonst in `COMPANY_PROPERTIES` anpassen): `legal_name`, `communication_name` (Fallback jeweils `name`), `address`, `zip`, `city`, `country`, `website`/`domain`, `description`, `invoice_email`, `invoice_name`, `vat_id`, `po_number`, `organization_type` (Vokabular), `partner_category` (Vokabular), `sponsoring_level`, `company_id` (Kundennummer). Line-Items brauchen `hs_sku` = Item-ID `I-nnnnn`.
...
- **Firmen-Eigenschaften** — Zuordnung **umgesetzt** in `lib/hubspot/mapping.ts` (11.09.; Portal-Eigenschaften gleichen Namens haben Vorrang, HubSpot-Eigenschaften sind der Fallback):
  ...
  | `customer_number` | `company_id` („Übergreifende Kundennummer (Company ID)“, Text) ✅ | **Entscheidung Konrad 24.09.2026** (ADM-057). Nicht `cheftreff_id_unternehmen` („synchronisiert mit Sevdesk und AirTable“, Zahl). Nur ergänzen, nie überschreiben; dieselbe Nummer an einer anderen Firma ist ein Gate-Fehler |
