...
- **Speaker-Foto nach Swapcard:** nicht geprüft — Konrads Testprofil hat kein Porträt (`has_photo = false`), der öffentliche Kopierweg blieb deshalb ungenutzt. Konrad lädt im Walkthrough eines hoch, danach nachziehen. **Seit SPK-047** liegt die Kopie im privaten Bucket, Swapcard bekommt eine signierte Adresse mit sieben Tagen Laufzeit, und jeder Echtlauf räumt Kopien ohne Speaker in der App weg. Nach dem ersten Echtlauf mit Porträt `node --env-file=.env.local scripts/swapcard-foto-hosts.mjs` fahren (nur lesend, nur Zählungen): `static.swapcard.com` heisst, Swapcard hat das Bild abgeholt und selbst abgelegt; steht dort unsere Supabase-Adresse, verlinkt Swapcard nur — dann braucht der Speaker-Lauf einen wöchentlichen Takt. Stand 25.09.: eine Person im Event, Foto auf `static.swapcard.com`, aber nicht aus unserem Lauf (noch kein Speaker-Lauf gegen live).
