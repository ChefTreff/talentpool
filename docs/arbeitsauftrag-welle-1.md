...
- Teil A: ✅ A4 Programm-Backend (+ Realtime aus der DB, Fragen-RPCs, Programmzeiten). ✅ A5 Bewerbungs-Mails + Cron (10.09., Migrationen 0020/0021, `/api/cron/mail`, Vercel Cron alle 10 Min.) — **in Betrieb seit 10.09. 11:00** (Resend-Keys, `CRON_SECRET`, Domain verifiziert). Test-Talente nur mit Resend-Testadressen (`delivered.dev`). ⏳ A1/A2 vivenu warten auf Sandbox-Key. A3 folgt mit A1. A6 teilweise: Mail/Cron-Tests in `v2_application_ticket.sql`, `v2_ingest.sql` mit A1.
- Teil B: PR #2 (B5, B2, B1) gemergt 10.09. (cbb345b); **PR #3 (B3 „Meine Teilnahme" unter `/meine`) gemergt 10.09. (8ad098b)**. B6 (Bewerbungs-Queue, Rollenverwaltung) war blockiert, weil der DB die Lesewege fehlten → Migration 0022 liefert `applications_for_session`, `applications_overview`, `roles_of_person`, `assign_role`, `revoke_role`, `search_people` (Signaturen: docs/datenmodell-v2.md, PR-#3-Kommentar). Nächster PR: **B6**; B4 nach A1/A2.
