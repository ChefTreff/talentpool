...
| K-37 | **Moderation in Swapcard?** Seit LEAD-042 (#203) können Stage Leads als Moderation eines Slots gewählt werden — ohne Speaker-Profil, Hub oder Ticket; sie erscheinen deshalb nicht im Swapcard-Export (der nimmt Profile). Sollen Moderationen (Stage Leads) in der Event-App am Programmpunkt stehen? Empfehlung: ja, als Speaker-Eintrag mit Rolle Moderation, sobald der Export um Personen ohne Profil erweitert wird (Admin-Chat, EA2) — sonst nein. **Nachtrag Admin-Chat 25.09. (#208):** technisch machbar als Erweiterung des Exports (`isSpeakerRoleOnPlannings` mit Rolle, Pass `crew-pass` für Bühnenleitungen). **Drei Entscheidungen vor dem Bau:** (a) welche Rollen aus `session_speaker` mitgehen — Moderation ja, „Host“ auch? (b) gilt die Einwilligungsregel „kommt mit der Zusage“ auch für externe Bühnenleitungen ohne Speaker-Profil? (c) woher kommt das Foto — Porträts hängen am Speaker-Profil (Vorschlag: Porträt-Upload im Stage-Lead-Portal, sonst Platzhalter). | neu 25.09. |
| K-38 | **Swapcard-Import: Nachweis mit Wegwerf-Person?** Um sicher auszuschließen, dass Swapcard beim Import eine Mail an neue Speaker schickt, würde der Admin-Chat eine Wegwerf-Person mit einer von uns mitgelesenen Adresse importieren und danach wieder löschen — das legt kurz etwas im Livekonto an. Freigeben? (Alternative: erster echter Import erst nach deiner Sichtung der Swapcard-Kampagnen im Backend.) | neu 25.09. |
