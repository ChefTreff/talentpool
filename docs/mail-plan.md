# Mail-Plan (Stand 10.09.2026, Migrationen bis 0035)
...
- **Einmaligkeit:** jede Vorlage genau einmal je Ereignis; die Erinnerung prüft `mail_log` (Vorlage × Person × Session), die anderen hängen an Zustandswechseln, die nur einmal passieren.
- **Interne Mails** gehen an alle aktiven `area_lead_speaker`, ohne solche an globale Admins (`notify_speaker_leads`).
