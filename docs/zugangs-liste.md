...
| `HUBSPOT_CLIENT_SECRET` | HubSpot | Signatur v3 der Webhooks `/api/webhooks/hubspot` (Deal → Phase „Onboarding Automation“) | Private App → Auth → Client secret; Webhook-Subscription `deal.propertyChange`/`dealstage` in derselben App; **Route erst mit eigener Domain erreichbar** (Deployment-Protection), bis dahin Sweep | 3 |
