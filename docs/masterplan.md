...
## Ergänzung v0.1c (08.09. abends) — Programm-Board mit Drag & Drop (Anforderung Konrad, Inventar §15)
- **Anforderung:** Ansicht wie der Tab „Master-Programm" (Tag × Bühnen × Zeit) im Speaker-Lead-Portal; Slots per Drag & Drop verschiebbar, damit das Programm visuell angepasst werden kann.
- **Komponente `ProgrammBoard`:** Spalten = Bühnen des gewählten Tages, vertikale Zeitachse, **5-Min-Snap**; Slot-Kachel zeigt Statusfarbe (aus Enum, nicht frei), Format, Titel, Speaker, Dauer. Interaktionen: Drag (Zeit und Bühne wechseln), Resize (Dauer), Klick → Detail-Drawer, **Backlog-Seitenleiste** (bestätigte Speaker/Partnerslots ohne Slot zum Hineinziehen), Undo, Konflikt-Hinweise (Überlappung auf der Bühne, Speaker zeitgleich doppelt, außerhalb Öffnungszeit, Fixblock, Wechselzeit unterschritten). Server: RPC `move_slot` prüft Scope + Constraints und schreibt `slot_history`; Supabase Realtime für gleichzeitige Bearbeiter; Druck-/Export-Ansicht speist Regieplan, App und Website.
...
## Ergänzung v0.1e (08.09. spät) — Produktstamm und Volunteer-Wiki (Inventar §16, §14.1)
- **Produktstamm:** `product` (sku PK `I-nnnnn`, name DE/EN, description, type package/addon/shop_item, category, unit, net_price, purchase_price_net, margin, vat_rate, supplier + supplier_sku/url, stock_total/sold, active, edition, Bilder; externe Schlüssel `hubspot_product_id`, `sevdesk_article_id`, `revenue_account`, `cost_centre` über `external_ref`) · `product_component` (Stückliste für Messebau/Regie) · `deliverable_template` je Produkt (Partner-Pflichten strukturiert, Fristregel). Import aus `docs/referenz/item-liste-2026.csv` in Welle 3 nach Bereinigung (Platzhalter, USt, Preise; Fragen 79–83). Danach ist das Portal Quelle der Wahrheit; HubSpot-Produkte und SevDesk-Artikel werden per SKU abgeglichen.
- **Volunteer-Wiki:** Import der Rollen-Seiten als jahresunabhängige Templates (Team-Blöcke → Rollen aus `role_assignment`, Termine/Orte → Variablen). `kb_article` erhält zusätzlich `role[]` und `phase` (vor Event / Aufbau / Event / Abbau) für Volunteers.
- **Event-App (08.09. abends):** Option wird vor Welle 3 erneut geprüft (Swapcard vs. Conferras, Frage 86). Bis zur Entscheidung wird die App-Anbindung als austauschbarer Adapter (`integration`-Schicht: Personen, Sessions, Aussteller, Barcode) gebaut, nicht Swapcard-spezifisch.
