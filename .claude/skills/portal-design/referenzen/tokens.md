...
- **`--ct-accent` steht im Code auf `#6D6DEF`, das Brandbook Final nennt `#6262DC`.** Solange das nicht entschieden ist, gilt im Code der Code — Komponenten benutzen die Klasse, nicht den Hex-Wert, deshalb ist der Wechsel später eine Zeile in `globals.css`. Entscheidung und Eintrag ins Entscheidungslog macht die Architektur-Session.
- **`text-muted-soft` (`#8A94A6`) erreicht auf Weiß nur 3,06:1** und trägt damit keinen lesbaren Text. Für Text `text-muted` nehmen; `muted-soft` bleibt für deaktivierte Zustände und rein dekorative Zeichen (so verwendet in `components/ui/Stepper.tsx`). Zwei Stellen benutzen es heute für echten Inhalt und gehören korrigiert: `app/(admin)/admin/vokabular/page.tsx:67` und `app/(admin)/admin/personen/[id]/page.tsx:204`. Platzhalter in Feldern sind in Ordnung — `Input`/`Textarea` setzen `placeholder:text-muted`.
