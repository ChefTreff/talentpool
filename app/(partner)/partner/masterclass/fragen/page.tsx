...
import { ladeFragen, ladeMasterclass } from "../daten";
import { EigeneFrageAntrag } from "../EigeneFrageAntrag";
import { FragenAuswahl } from "../FragenAuswahl";
import { MasterclassKopf } from "../MasterclassKopf";

export const dynamic = "force-dynamic";

/**
 * Bewerbungsfragen der Masterclass (PART-045), vierter Reiter: was das Team
 * gesetzt hat (nur lesen), Katalogfragen, die Partner wählen dürfen, und bis
 * zu zwei eigene Fragen, die das Programm-Team freigibt.
 *
    ...
    <>
      ...
      <div className="flex flex-col gap-6">
        {sessions.map((x, i) => {
          ...
          return (
            <Card key={x.id}>
              <CardHeader title={titel} description={s.questionsLead} />
              <div className="flex flex-col gap-6">
                {team.length > 0 && (
                ...
                <section aria-label={s.ownTitle}>
                  ...
                  <div className="mt-3">
                    {eigene.length >= MAX_EIGENE_FRAGEN ? (
                      ...
                      <EigeneFrageAntrag sessionId={x.id} t={{ ...s, cancel: t.partnerTalk.cancel }} rpcMessages={t.rpc} />
                    ) : null}
                  </div>
                </section>
              </div>
